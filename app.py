...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2[['Category','App','Reviews','Rating']]\
            .sort_values(by = 'Reviews', ascending = False).head(10).reset_index(drop=True).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    plt.barh(X1,Y1, color=my_colors)
    ...
    ## Scatter Plot
    X2 = df2["Reviews"].values # axis x
    Y2 = df2['Rating'].values # axis y
    area = playstore['Installs'].values/10000000 # ukuran besar/kecilnya lingkaran scatter plot
    fig = plt.figure(figsize=(5,5),dpi=300)
    fig.add_subplot()
    # isi nama method untuk scatter plot, variabel x, dan variabel y
    plt.scatter(x=X2,y=Y2, s=area, alpha=0.3)
    plt.xlabel('Reviews')
    plt.ylabel('Rating')
    ...
    X3=(df2["Size"]/1000000).values
    fig = plt.figure(figsize=(5,5))
    fig.add_subplot()
    plt.hist(X3,bins=100, density=True,  alpha=0.75)
    plt.xlabel('Size')
    ...
    plt.legend(labels=legend, title='Legend', loc='upper center', bbox_to_anchor=(0.5, 1), ncol=2)
    plt.axis('equal')
    #plt.title('Total App Distribution According to Content Rating')
    plt.savefig('cr_dist.png', bbox_inches="tight")
    figfile = BytesIO()
    plt.savefig(figfile, format='png')
    figfile.seek(0)
    figdata_png = base64.b64encode(figfile.getvalue())
